"""ComboV2_MHA: S1(SSL+raw)+MHA→task1, S2 sees [S1|S2]+MHA→task2-5"""
...
X_w=[]
y_w={k:[] for k in y_case}
for i in range(177):
    nw=36 if y_case['task5'][i]<100 else 12
    ts=X_ds[i]
    T=ts.shape[0]
    tp=max(0,T-200)
    for _ in range(nw):
        t0=np.random.randint(0,max(1,tp)) if tp>0 else 0
        w=ts[t0:t0+200]
        if w.shape[0]<200:
            w=np.pad(w,((0,200-w.shape[0]),(0,0)))
        X_w.append(w)
    for k,v in y_case.items():
        y_w[k].extend([v[i]]*nw)
X_w=np.stack(X_w).astype(np.float32)
y_w={k:np.array(v) for k,v in y_w.items()}
...
class CNNE(nn.Module):
    def __init__(self):
        ...
        for ch,k in zip([64,128,128],[7,5,3]):
            ls.extend([nn.Conv1d(c,ch,k,padding=k//2),nn.BatchNorm1d(ch),nn.ReLU(),nn.MaxPool1d(2),nn.Dropout(0.3)])
            c=ch
...
ssl_enc=CNNE().to(device)
ssl_enc.load_state_dict(torch.load('models/cross_ssl.pt','cpu'))
for p in ssl_enc.parameters():
    p.requires_grad=False
A5=torch.tensor([0.,25.,50.,75.,100.])

class ComboV2_MHA(nn.Module):
    def __init__(self):
        super().__init__()
        self.s1_ssl=ssl_enc
        self.s1_raw=nn.Sequential(nn.Conv1d(7,32,5,padding=2),nn.ReLU(),nn.AdaptiveAvgPool1d(1))
        self.s1_mha=nn.MultiheadAttention(160,4,batch_first=True)
        self.s1_head=nn.Linear(160,1)
        self.s2=CNNE()
        self.fusion_mha=nn.MultiheadAttention(288,4,batch_first=True)
        self.fusion=nn.Sequential(nn.Linear(288,128),nn.BatchNorm1d(128),nn.ReLU(),nn.Dropout(0.2))
        self.out_mha=nn.MultiheadAttention(128,4,batch_first=True)
        self.h2=nn.Linear(128,4)
        self.h3=nn.Linear(128,9)
        self.h4=nn.Linear(128,5)
        self.ac=nn.Linear(128,5)
        self.dh=nn.ModuleList([nn.Sequential(nn.Linear(128,32),nn.ReLU(),nn.Linear(32,1))for _ in range(5)])
    def forward(self,x):
        h1=self.s1_ssl(x)
        r1=self.s1_raw(x.transpose(1,2)).squeeze(-1)
        f1=torch.cat([h1,r1],dim=1)
        q1=f1.unsqueeze(1);c1,_=self.s1_mha(q1,q1,q1);f1=c1.squeeze(1)
        o1=self.s1_head(f1)
        h2=self.s2(x)
        f2=torch.cat([f1,h2],dim=1)
        q2=f2.unsqueeze(1);c2,_=self.fusion_mha(q2,q2,q2);f2=c2.squeeze(1)
        hu=self.fusion(f2)
        q3=hu.unsqueeze(1);c3,_=self.out_mha(q3,q3,q3);hu=c3.squeeze(1)
        o2=self.h2(hu)
        o3=self.h3(hu)
        o4=self.h4(hu)
        lg=self.ac(hu)
...
Xt=zscore(X_test)[:,::6,:][:,:200,:].astype(np.float32)
Xt_t=torch.from_numpy(Xt).float().to(device)
best_r2=-999
bep=0
bck=None
print('ComboV2_MHA: 400ep...')
for ep in range(400):
    m.train()
    t=0
    n=0
    for batch in ldr:
        x=batch['x'].to(device)
        t5=batch['task5'].float().to(device)
        t5s=t5.squeeze(-1)
        o1,o2,o3,o4,o5,lg=m(x)
        dif=torch.abs(t5s.unsqueeze(1)-A5.to(device).unsqueeze(0))
        tc=dif.argmin(1)
    ...
    with torch.no_grad():
        _,_,_,_,o5,_=m(Xt_t)
        r2=R2(gt_t5,o5.cpu().numpy().squeeze())
    if r2>best_r2:
        best_r2=r2
        bep=ep+1
        bck={k:v.cpu().clone()for k,v in m.state_dict().items()}
    if(ep+1)%40==0:
        print(f'  Ep{ep+1}:tr={t/n:.1f} r2={r2:.4f} best={best_r2:.4f}@{bep}')

m.load_state_dict(bck)
m.eval()
with torch.no_grad():
    o1,o2,o3,o4,o5,_=m(Xt_t)
    p1=(torch.sigmoid(o1)>0.5).int().cpu().numpy().squeeze()
    p2=o2.argmax(1).cpu().numpy()
    p3=o3.argmax(1).cpu().numpy()
    p4=o4.argmax(1).cpu().numpy()
    p5=o5.cpu().numpy().squeeze()
gt={c:ans[c].values for c in['task1','task2','task3','task4','task5']}
print(f'\nComboV2_MHA best@{bep}')
...
for sc in[1,4]:
    msk=ans['Spacecraft No.'].values==sc
    print(f'  SC{sc}:mae={MAE(gt["task5"][msk],p5[msk]):.1f} r2={R2(gt["task5"][msk],p5[msk]):.4f}')
print('DONE')
